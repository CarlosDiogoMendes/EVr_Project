...
include_once './objects/PedidoEmpresa.php';

$database = new Database();
$db = $database->getConnection();
$empresa = new Empresa($db);
$pais = new Pais($db);
$pedidoEmpresa = new PedidoEmpresa($db);
...
if ($data == null || $data->email == null || $data->password == null ||
    ...
    $pedidoEmpresa->emailEmpresa = $data->email;
    $stmt = $pedidoEmpresa->insert();
    $error = $stmt->errorInfo();
    echo $error[2];
    if (isset($error[2])) {
        echo '"message":"Failed to create enterprise registration request"';
    } else {
        $empresa->email = $data->email;
        $empresa->password = $data->password;
        $empresa->nome = $data->nome;
        $empresa->telefone = $data->telefone;
        $empresa->pais = $empresa->pais . getCodeFromCountry($data->pais);
